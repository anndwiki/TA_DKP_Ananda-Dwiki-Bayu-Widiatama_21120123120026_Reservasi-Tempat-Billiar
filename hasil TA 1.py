...
class ReservationSystem:
    ...
    def create_main_menu(self):
        ...
        ttk.Label(main_frame, text="Sistem Reservasi Biliar", font=("Arial", 16), background="lightblue").grid(row=0, column=0, columnspan=2, pady=20)
    ...
    def open_make_reservation(self):
        ...
        self.new_window.configure(background="lightgreen")
    ...
    def create_reservation_widgets(self, window):
        ...
        ttk.Label(main_frame, text="Tanggal (YYYY-MM-DD):", style="Reservation.TLabel").grid(row=2, column=0, sticky=tk.W, padx=10, pady=5)
        ...
        ttk.Radiobutton(main_frame, text="Ruangan Biliar Premium", variable=self.ruangan, value="Ruangan Biliar Premium", style="Reservation.TRadiobutton").grid(row=6, column=2, sticky=tk.W, padx=10)
        
        ttk.Label(main_frame, text="Tongkat Biliar:", style="Reservation.TLabel").grid(row=7, column=0, sticky=tk.W, padx=10, pady=5)
        self.stick = tk.StringVar()
        self.stick.set("Tongkat Biliar Reguler")
        ttk.Radiobutton(main_frame, text="Tongkat Biliar Reguler", variable=self.stick, value="Tongkat Biliar Reguler", style="Reservation.TRadiobutton").grid(row=7, column=1, sticky=tk.W, padx=10)
        ttk.Radiobutton(main_frame, text="Tongkat Biliar Premium", variable=self.stick, value="Tongkat Biliar Premium", style="Reservation.TRadiobutton").grid(row=7, column=2, sticky=tk.W, padx=10)
        ...
        ttk.Radiobutton(main_frame, text="Tunai", variable=self.payment_method, value="Tunai", style="Reservation.TRadiobutton").grid(row=8, column=1, sticky=tk.W, padx=10)
        ttk.Radiobutton(main_frame, text="Transfer", variable=self.payment_method, value="Transfer", style="Reservation.TRadiobutton").grid(row=8, column=2, sticky=tk.W, padx=10)
        ttk.Radiobutton(main_frame, text="Scan QR", variable=self.payment_method, value="Scan QR", style="Reservation.TRadiobutton").grid(row=8, column=3, sticky=tk.W, padx=10)
    ...
    def display_prices(self, window):
        ...
        self.price_label = ttk.Label(window, text=price_list, justify=tk.LEFT, background="lightgreen")
    ...
    def display_reservations(self, window):
        ...
        self.reservation_output = tk.Text(main_frame, width=80, height=20)
        self.reservation_output.insert(tk.END, reservations_details)
        self.reservation_output.config(state=tk.DISABLED)
        self.reservation_output.grid(row=0, column=0, columnspan=4, padx=10, pady=10)
...
if __name__ == "__main__":
    ...
    style = ttk.Style()
    style.configure("Main.TFrame", background="lightblue")
    style.configure("Main.TButton", font=("Arial", 12), padding=10)
    style.configure("Reservation.TFrame", background="lightgreen")
    style.configure("Reservation.TLabel", background="lightgreen", font=("Arial", 10))
    style.configure("Reservation.TRadiobutton", background="lightgreen")
    style.configure("Reservation.TButton", font=("Arial", 12), padding=10)
